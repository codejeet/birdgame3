...
export const Bird = React.memo(({ statsRef, onMove, isPaused, playFlapSound, rotationRef, teleportTarget, teleportRotation, frozen, raceStartPosition, mouseSensitivity = 1.0 }: BirdProps) => {
    ...
    useFrame((state, delta) => {
        ...
        const floorLimit = surfaceHeight + 1.0; // Bird's cone height is 2, so center at +1.0 means bottom touches ground
        ...
        if (mode.current === 'flying') {
            ...
            if (position.current.y <= floorLimit) {
                ...
                // Mobile Tap Super Jump
                if (controls.current.mobileTap) {
                    velocity.current.y = 80; // Super jump to clear ground and start flying
                    playFlapSound();
                }
            }
