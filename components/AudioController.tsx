...
export const AudioController = forwardRef<AudioHandle, AudioControllerProps>(({ isPaused }, ref) => {
  ...
  const ringMissRef = useRef<HTMLAudioElement | null>(null);
  ...
  useImperativeHandle(ref, () => ({
    ...
    playRingMiss: () => {
        if (ringMissRef.current) {
            ringMissRef.current.currentTime = 0;
            ringMissRef.current.play().catch(() => {});
        }
    },
    playGameStart: () => {
    ...
    playGameOver: () => {
        if (ringMissRef.current) {
             // Re-use miss sound for now, or logic to play distinct
             ringMissRef.current.currentTime = 0;
             ringMissRef.current.play().catch(() => {});
        }
    }
  ...
  useEffect(() => {
    ...
    ringMissRef.current = new Audio('https://assets.mixkit.co/active_storage/sfx/257/257-preview.mp3'); // Retro failure
    ringMissRef.current.volume = 0.5;
    ...
    gameStartRef.current.volume = 0.5;
    ...
    const unlockAudio = () => {
        ...
        if (ringMissRef.current) ringMissRef.current.load();
