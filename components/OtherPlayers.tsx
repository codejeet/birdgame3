...
const RemoteBird = React.memo(({ player, battle }: { player: RemotePlayer, battle?: any }) => {
  ...
  const birdColor = useMemo(() => {
    if (player.team === 'red') return '#ff4444';
    if (player.team === 'blue') return '#4444ff';
  ...
  return (
    <group ref={groupRef} position={player.position}>
      ...
      >
        ...
        {battle?.isActive && player.hp !== undefined && ` ❤️${Math.ceil(player.hp)}`}
      </Text>

      {/* HP Bar */}
      {battle?.isActive && player.hp !== undefined && (
        <group position={[0, 3.2, 0]}>
          <mesh position={[0, 0, 0]}>
            <planeGeometry args={[2, 0.2]} />
            <meshBasicMaterial color="black" />
          </mesh>
          <mesh position={[(player.hp / 100) - 1, 0, 0.01]} scale={[player.hp / 100, 1, 1]}>
            <planeGeometry args={[2, 0.15]} />
            <meshBasicMaterial color={player.hp < 30 ? "red" : "green"} />
          </mesh>
        </group>
      )}
...
export const OtherPlayers: React.FC<OtherPlayersProps> = React.memo((props) => {
  const { players, localPlayerId } = props;
  ...
  return (
    <group>
      {remotePlayers.map(player => (
        <RemoteBird key={player.id} player={player} battle={props.battle} />
      ))}
    </group>
  );
});
