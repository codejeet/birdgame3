...
export const RingManager: React.FC<RingManagerProps> = React.memo(({ birdPosition, birdRotation, statsRef, audioRef, isPaused, targetRingRef }) => {
  ...
  useFrame((state, delta) => {
    ...
    // 1. Init logic (Only if empty and not active - usually for initial load)
    ...
    if (targetRing) {
        ...
        if (distSq < hitRadius * hitRadius) {
            ...
            if (targetRing.type === 'starter') {
                ...
                statsRef.current.combo = 0;
                statsRef.current.feathers = MINIGAME_START_LIVES;
             ...
             targetRing.passed = true; 
             if (gameActive.current) {
                 statsRef.current.feathers -= 1;
                 statsRef.current.combo = 0;
                 audioRef.current?.playRingMiss();
                 dirty = true;
                 
                 if (statsRef.current.feathers <= 0) {
                     endGame();
                 }
             } else {
                 spawnStarterRing();
